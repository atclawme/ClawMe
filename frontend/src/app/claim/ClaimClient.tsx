  ...
  const checkAvailability = useCallback(
    async (value: string) => {
      if (!value) {
        ...
        setReservedForYou(false)
        return
      }
      const v = validateHandle(value)
      if (!v.valid) {
        setAvailability('invalid')
        setHandleError(v.error || 'Invalid')
        setReservedForYou(false)
        return
      }
      ...
      try {
        ...
        const res = await fetch(
          `/api/waitlist/check?handle=${encodeURIComponent(value)}${emailParam}`
        )
    ...
    <main className="min-h-screen flex items-center justify-center px-6 bg-[#0A0A0F]">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1
            className="text-[36px] font-bold text-[#F0F0F5] mb-3"
        ...
        <div className="rounded-xl p-8" style={{ backgroundColor: '#13131A', border: '1px solid #27272F' }}>
          <div className="mb-5">
            <label
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                onFocus={(e) => {
                  if (!borderColor) e.currentTarget.style.borderColor = '#6C47FF'
                }}
                onBlur={(e) => {
                  if (!borderColor) e.currentTarget.style.borderColor = '#3F3F50'
                }}
              />
